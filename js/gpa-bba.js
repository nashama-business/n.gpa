  ...
  for (let i = 1; i <= count; i++) {
    ...
    coursesection.innerHTML = `
      ...
      <div class="button-group course-hours" id="course-hours-${i}">
        <button class="option-button" data-value="0">0</button>
        <button class="option-button" data-value="0.5">0.5</button>
      ...
      <section class="repeated-course hidden">
        ...
        <section class="old-course hidden">
          ...
          <label>رمز المادة القديم للمادة المعادة حسب النظام القديم:</label>
          <div class="button-group old-system-code" id="old-system-code-${i}">
            <button class="option-button" data-value="د">د</button>
            <button class="option-button" data-value="د-">د-</button>
            <button class="option-button" data-value="هـ">هـ</button>
          </div>
    ...
    const oldSystemCodeGroup = coursesection.querySelector('.old-system-code');
    ...
    // عند اختيار رمز قديم، يتم إلغاء تحديد رموز النظام القديم والعكس
    oldCourseCodeGroup.addEventListener('click', () => {
      const buttons = oldCourseCodeGroup.querySelectorAll('.option-button');
      buttons.forEach(btn => {
        if (btn.classList.contains('selected')) {
          oldSystemCodeGroup.querySelectorAll('.option-button').forEach(systemBtn => {
    ...
    oldSystemCodeGroup.addEventListener('click', () => {
      const buttons = oldSystemCodeGroup.querySelectorAll('.option-button');
      buttons.forEach(btn => {
        if (btn.classList.contains('selected')) {
          oldCourseCodeGroup.querySelectorAll('.option-button').forEach(courseBtn => {
    ...
    repeatedGroup.addEventListener('click', () => {
      if (getSelectedValue(repeatedGroup.id) === 'yes') {
        ...
        oldSystemCodeGroup.querySelectorAll('.option-button').forEach(btn => {
          btn.classList.remove('selected');
        });
      }
    });
    ...
    courses.push({
      ...
      oldSystemCode: coursesection.querySelector('.old-system-code')
    });
  }
...
function calculateGPA() {
  ...
  // جلب حالة الطالب ونوع الفصل والتخصص
  ...
  // منطق تحديد حالة الطالب النهائية حسب المعدل والنظام
